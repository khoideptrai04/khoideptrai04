...
router.get("/", authenticateEmployee(), async (req, res) => {
    ...
    let dashboardData = {
        users: [],
        orders: { monthly: [], breakdown: [], total: 0 },
        products: []
    };
    ...
    try {
        const [userDistribution, monthlyTrend, topProducts, orderStats] = await Promise.all([
            Users.getUserTypeDistribution(),
            Orders.getMonthlyTrend(6),
            Orders.getTopSellingProducts(8),
            Orders.getStatusTotals()
        ]);
        ...
        dashboardData = {
            ...
            orders: {
                ...
                breakdown: orderStats.breakdown,
    ...
    res.render(`${config.views}/dashboard/index.pug`, {
        products: products,
        dashboardJson: JSON.stringify(dashboardData || {})
    });
});
...
router.get("/products", authenticateEmployee(), async (req, res) => {
    ...
    try {
        const rows = await Products.getAllWithSizes();
        const grouped = {};
        rows.forEach(row => {
...
router.get("/orders", authenticateEmployee(), async (req, res) => {
    const OrdersController = require('../controllers/orders.js');
    const Orders = new OrdersController();
    ...
    let ordersList = { orders: [], pagination: { page: 1, pages: 1, total: 0 } };
    let stats = { totalOrders: 0, breakdown: [] };

    try {
        ordersList = await Orders.getAdminList({ page, status, search, limit: 12 });
        stats = await Orders.getStatusTotals();
    } catch (err) {
        console.log(err);
    }

    res.render(`${config.views}/dashboard/orders.pug`, {
        orders: ordersList.orders,
        pagination: ordersList.pagination,
        filters: { status, search },
        stats,
        statusOptions: OrdersController.ORDER_STATUSES
    });
});

router.get("/orders/:id", authenticateEmployee(), async (req, res) => {
    const OrdersController = require('../controllers/orders.js');
    const Orders = new OrdersController();
    let orderDetail;

    try {
        orderDetail = await Orders.getOrderDetail(req.params.id);
    } catch (err) {
        return res.status(404).render(`${config.views}/dashboard/orderDetail.pug`, {
            error: 'Order not found'
        });
    }

    res.render(`${config.views}/dashboard/orderDetail.pug`, {
...
router.post("/orders/:id/status", authenticateEmployee(), async (req, res) => {
    const OrdersController = require('../controllers/orders.js');
    const Orders = new OrdersController();
    ...
    try {
        ...
        res.status(400).json({ success: false, message: err.message || 'Unable to update status' });
    }
});
