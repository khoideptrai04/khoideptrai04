...
const controller = class ProductsController {
    ...
    getAllWithSizes() {
        return new Promise((resolve, reject) => {
            const sql = `
                ...
                ORDER BY products.id ASC, FIELD(sizes.size, 'LARGE','MEDIUM','SMALL'), sizes.size
            `;
    ...
    /**
     * Lấy thông tin size + giá cho danh sách sản phẩm
     * @param {Array<number>} productIds
     * @returns {Promise<Array>}
     */
    getSizesForProducts(productIds) {
        return new Promise((resolve, reject) => {
            ...
            this.con.query(
                'SELECT product_id, size, price FROM sizes WHERE product_id IN (?)',
                [productIds],
                function (err, result) {
                    if (err) {
                        reject(err);
